...
else
{
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		if ($row['promote_price'] > 0)
		{
		   	$arr[$row['goods_id']]['sheng_price'] = price_format($row['market_price']-$row['promote_price']);
			$arr[$row['goods_id']]['cuxiao_zhekou_price'] = round(($row['promote_price']/$row['market_price'])*100)/10+"";
		}
		else{
             $arr[$row['goods_id']]['cuxiao_zhekou_price'] = round(($row['shop_price']/$row['market_price'])*100)/10+"";
			$arr[$row['goods_id']]['sheng_price'] = price_format($row['market_price']-$row['shop_price']);
        }
		...
		$arr[$row['goods_id']]['pics']   = get_goods_gallery($row['goods_id']);
    }
	...
	{
	  $smarty->display('search_promotion.dwt');
	}
	else
	{
     $smarty->display('search.dwt');
