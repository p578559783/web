...
function get_category_recommend_goods($type = '', $cats = '', $cat_num = 0,$brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price,g.salenum, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
        ...
        else
        {
		 ...
		 /*加入省多少*/	
			if ($row['promote_price'] > 0)
			{
				...
				$goods[$idx]['cuxiao_zhekou_price'] = round(($row['promote_price']/$row['market_price'])*100)/10+"";
			}
			elseif($row['shop_price']>0 and $row['market_price']>0){
				$goods[$idx]['cuxiao_zhekou_price'] = round(($row['shop_price']/$row['market_price'])*100)/10+"";
				$goods[$idx]['sheng_price'] = price_format($row['market_price']-$row['shop_price']);
			} 
			else{
			   $goods[$idx]['cuxiao_zhekou_price']=0;
			   $goods[$idx]['sheng_price'] =0;
			}
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		if ($row['promote_price']>0 )
		{
		   $row['sheng_price'] = $row['market_price']-$row['promote_price'];
            $row['cuxiao_zhekou_price'] = round(($row['promote_price']/$row['market_price'])*100)/10+"";		    
		}
		else{
		   $row['sheng_price'] = $row['market_price']-$row['shop_price'];
		   $row['cuxiao_zhekou_price'] = round(($row['shop_price']/$row['market_price'])*100)/10+""; 	
        }
